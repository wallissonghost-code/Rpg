...
export function updatePlayer(dt){
 ...
 if(!state.openBag&&!state.mapOpen&&(i.x||i.y)){const l=Math.hypot(i.x,i.y)||1,dx=i.x/l,dy=i.y/l;p.x+=dx*CONFIG.player.speed*dt;p.y+=dy*CONFIG.player.speed*dt;p.dirX=dx;p.dirY=dy}
...
export function attack(){
 const p=state.player;if(p.attackCd>0||state.openBag||state.mapOpen)return;p.attackCd=CONFIG.player.attackCooldown;
